...
permit event-strings which are not well-formed.%where either the interval-like 
%constraint or the consistency constraint might be broken, which are not 
...
\noindent
The TLINK nodes will have either a \texttt{timeID} or \texttt{eventInstanceID} 
attribute, as well as either a \texttt{relatedToTime} or 
\texttt{relatedToEventInstance} attribute, as well as a \texttt{relType} 
attribute. The Allen Relation specified by \texttt{relType} will correspond to 
one of the 13 event-strings given in Section \ref{event rep}, which will 
in 
turn determine which formula the new event-string will satisfy. The examples 
above give the event-strings \ebox{}\ebox{ei9}\ebox{t1, ei9}\ebox{t1}\ebox{}, 
and \ebox{}\ebox{ei9}\ebox{}\ebox{ei10}\ebox{}, respectively.\footnote{Using 
	https://www.scss.tcd.ie/$\sim$dwoods/timeml/ to quickly extract TLINKs from 
	a 
	TimeML document and translate them to event-strings}
