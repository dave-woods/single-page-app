...
export default class Header extends React.Component {
  ...
  toggleNavOpen () {
    ...
    if (nav.classList.contains('navbar-collapsed')) { nav.classList.toggle('navbar-collapsed-open') }
  }

  resizeNavbar () {
    const nav = document.querySelector('.navbar')
    if (window.innerWidth <= 400 && !nav.classList.contains('navbar-collapsed')) { nav.classList.add('navbar-collapsed') } else if (window.innerWidth > 400 && nav.classList.contains('navbar-collapsed')) { nav.classList.remove('navbar-collapsed') }
  }
  ...
  render () {
    ...
    return (
      <nav className={'navbar ' + ((window.innerWidth <= 400) ? 'navbar-collapsed' : '')}>
        <button onClick={this.toggleNavOpen.bind(this)}><span className='menu-button-bar' /><span className='menu-button-bar' /><span className='menu-button-bar' /></button>
        <ul>
